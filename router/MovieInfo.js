...
router.post("/movie-info/add", (req, resp) => {
  ...
  // 表单验证通过，执行添加操作
  let sql = `insert into movie_info (category_id,
    cover,
    title,
    type,
    star_actor,
...
router.get("/movie-infos", async (req, resp) => {
  ...
  // TODO 服务器表单验证 如果验证通过那么继续后续业务 如果表单不通过 则直接返回参数异常
  let schema = Joi.object({
...
router.post("/movie-infos/name", async (req, resp) => {
  ...
  // TODO 服务器表单验证 如果验证通过那么继续后续业务 如果表单不通过 则直接返回参数异常
  let schema = Joi.object({
    name: Joi.string().required(), // page必须是数字，必填
    page: Joi.number().required(), // page必须是数字，必填
    pagesize: Joi.number().integer().max(100).required(), // pagesize必须是不大于100的数字，必填
  });
  let { error, value } = schema.validate(req.body);
...
router.post("/movie-info/update", (req, resp) => {
  ...
  // 表单验证通过，执行更新电影信息操作
  let sql = `update movie_info set category_id=?,
  cover=?,
  title=?,
  type=?,
  star_actor=?,
...
router.post("/movie-info/bind-actors", async (req, resp) => {
  ...
  try {
    ...
    // 执行sql，将movie_id与actor_ids绑定在一起，全部插入数据库
    let params = "";
    let paramsArray = [];
